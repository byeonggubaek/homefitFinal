...
const WdogTable = ({
  ...
  const visibleColumns = columns.filter(
    (column) => String(column.COL_NAME ?? "").trim() !== ""
  );
          ...
          <TableRow>
            {visibleColumns.map((column) => {
              ...
              return (
                <TableHead
                  key={column.COL_ID}
                  className={classAdjust}
                  style={{ width: column.COL_WIDTH }}
                >
                  {column.COL_NAME}
                </TableHead>
              );
          ...
          {records.map((record, index) => (
            ...
            >
              {visibleColumns.map((column) => {
                ...
                return (
                  <TableCell key={`${column.COL_ID}-${index}`} className={classAdjust}>
                    <span className={listValue ? listValue : ""}>{cellValue}</span>
                  </TableCell>
                );
              })}
            </TableRow>
