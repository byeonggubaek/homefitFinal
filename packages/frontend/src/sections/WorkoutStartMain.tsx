...
const WorkoutStartMain: React.FC<{ wor_id: string | null }> = ({ wor_id }) => {
  ...
  const [detectedExercise, setDetectedExercise] = useState<string>(""); // 🌟 AI가 판별한 운동을 화면에 표시하기 위한 상태
  ...
  const calculateAngle = (a: any, b: any, c: any) => {
    if(a.visibility < 0.5 || b.visibility < 0.5 || c.visibility < 0.5) return 180; // 신뢰도 낮으면 각도 계산 안 함
  ...
  const handleCount = (angle: number, downLimit: number, upLimit: number) => {
    ...
    if (angle < downLimit) {
      ...
      setDetectedExercise(currentAiExerciseRef.current); // 🌟 AI가 판별한 운동을 화면에 표시
  ...
  const startDetection = () => {
    setIsDetecting(true);
    setCount(0);
    setDetectedExercise(""); // 🌟 AI가 판별한 운동을 화면에 표시    
    setPlankTime(0);
        ...
        <Card className="overflow-hidden border-none shadow-xl">
          <CardHeader className="bg-white pb-4">
            ...
            <div className='flex justify-between'>
              <div>
                  {detectedExercise && (
                    <p className="mt-2 text-2xl text-green-600 font-bold">
                      {detectedExercise}
                    </p>
                  )}
              </div>
